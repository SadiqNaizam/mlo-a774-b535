...
const defaultBook: BookData = {
  ...
  pages: [
    ...
    "'Pre-warp? Out here?' Eva mused, turning to her first officer. 'What do you make of it, Jian?' Jian, ever the pragmatist, shrugged. 'Could be anything. A ghost signal, a natural phenomenon, or a trap.' His gruff voice was a familiar comfort.",
    "Eva smiled faintly. 'Always the optimist. Keep the shields at 70% and power the long-range comms array. If there's someone down there, I want to be able to say hello.' As she spoke, a new, smaller light blinked on the star chart. A ship. And it wasn't one of theirs.",
  ],
};

const ImmersiveReader: React.FC<ImmersiveReaderProps> = ({ book = defaultBook }) => {
  ...
  useEffect(() => {
    console.log('ImmersiveReader loaded for book:', book.title);
  }, [book.title]);
  ...
  const togglePlay = () => {
    ...
    console.log(isPlaying ? 'Pausing TTS' : 'Playing TTS');
  };
      ...
      >
        ...
        <footer className="w-full p-4 flex flex-col items-center gap-4">
          ...
          <div className="flex items-center justify-center gap-4 p-3 rounded-full bg-white/70 dark:bg-black/70 backdrop-blur-sm shadow-lg border border-stone-200 dark:border-stone-800">
            ...
            {/* Character Voice Mapper Trigger */}
            <Dialog>
              <Tooltip>
                <TooltipTrigger asChild>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  <DialogTitle>Character Voice Mapper</DialogTitle>
                  <DialogDescription>
                    Assign a unique voice to each character for an immersive audio experience.
                  </DialogDescription>
                </DialogHeader>
